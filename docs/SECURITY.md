...
- **Начальный админ** (identity seed): при первом старте создаётся пользователь из **`ADMIN_USERNAME`** / **`ADMIN_PASSWORD`** / **`ADMIN_EMAIL`** (по умолчанию `admin` / `admin` / `admin@local.dev`). В проде задайте свои значения и смените пароль сразу после первого входа.
